...
async function assertManifest() {
    ...
    assert.equal(manifest.name, 'react-reflect');
    assert.equal(manifest.type, 'module');
    assert.equal(manifest.bugs.url, 'https://github.com/enormora/react-reflect/issues');
    assert.equal(manifest.homepage, 'https://github.com/enormora/react-reflect#readme');
    assert.equal(manifest.exports['.'].import, './react-reflect.entry-point.js');
    assert.equal(manifest.exports['.'].types, './react-reflect.entry-point.d.ts');
}

async function assertRuntimeImport() {
    const packageExports = await import(pathToFileURL(path.join(packageFolder, 'react-reflect.entry-point.js')).href);
    const React = await import('react');

    assert.equal(typeof packageExports.createFakeRefNode, 'function');
    assert.equal(typeof packageExports.matchRefs, 'function');
    assert.equal(typeof packageExports.reflect, 'function');
